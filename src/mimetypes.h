...
// Builds the extension -> MIME-type hash chains. Call once at startup
// before any request serves a file.
void init_mimetypes(void);

// Resolve the MIME type from the filename extension (case-insensitive).
// Returns DEFAULT_CONTENT_TYPE ("application/octet-stream") when unknown.
StrSlice get_mimetype(char* filename);
